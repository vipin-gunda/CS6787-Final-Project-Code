# NOTE: THIS IS THE ATM HEAD PROVIDED BY SEGVIT. IN THIS FILE, WE ONLY CHANGED LINES 190 to 205. 
...
class ATMHead(BaseDecodeHead):
    def __init__(
        ...
        # Change line below based on whether unseen classes are included
         self.q.weight = nn.Parameter(torch.load("/home/vg245/SegVit/language_embeddings_allclasses.pth"), requires_grad = False)
        
        # End update

        self.class_embed = nn.Linear(dim, self.num_classes + 1, bias = False)
        # Changing linear layer to have CLIP 
        
        loaded_weights = torch.load("/home/vg245/SegVit/language_embeddings_allclasses.pth")
        extended_weights = torch.cat([loaded_weights, torch.zeros(1, 512)], dim=0)
        
        transposed_weights = extended_weights.T
        self.class_embed.weight = torch.nn.Parameter(extended_weights, requires_grad=False)
